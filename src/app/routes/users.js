...
export function registerRoutes(app) {
  ...
  app.get('/api/portal/identity', async (req, res) => {
    ...
    try {
      // Look up user's SCIM profile — returns { displayName, groupRole }
      ...
      if (user) {
        ...
        // Backfill display_name from SCIM if it looks like an email-derived fallback
      ...
      // New user — resolve SCIM name + role
      ...
      // Use SCIM displayName if available; fall back to email-local-part prettification
      ...
      console.info(`[identity] Auto-registered ${email} as ${role} (name: ${displayName})`);
