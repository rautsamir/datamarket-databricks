...
# psql — needed for Lakebase schema init and SP grants
# Optional but strongly recommended; without it the schema step is skipped
...
| 2 | Reads workspace host and your email from CLI profile |
| | Admin email **auto-detected** from your profile (no flag needed) |
| 3 | Looks up Lakebase project → **creates it if it doesn't exist** (~2–3 min first time) |
...
| 6 | Uploads source code and built assets to your workspace, deploys the app |
| 7 | Creates Lakebase schema, applies `schema.sql`, grants SP full access |
| | Seed data applied automatically when `--demo-mode true` |
| 8 | **Auto-detects** a SQL Warehouse, grants SP `CAN_USE` |
...
| `--warehouse-id` | **auto-detected** from workspace | SQL Warehouse ID — script picks the best running warehouse automatically |
...
| `--seed` | auto (`true` when demo-mode, `false` otherwise) | Apply `schema/seed.sql` — loads demo products, users, and requests |
...
> Branding (logo, app name, colour), integrations (Slack, email), and feature toggles are all configured via **Manage → Settings** in the UI — no redeployment needed.
...
### Configure via Settings (UI — no redeploy)

| Setting | Where |
|---|---|
| SQL Warehouse ID | Manage → Settings → Integrations (auto-set by script if detected) |
...
| Search chips, About/FAQ/Contact pages | Manage → Settings → respective sections |

### Import your data catalog

In **Manage → Data Products → Import from Unity Catalog**, browse your catalog → schema → tables and import. Users can't discover anything until at least one data product is imported.
...
| Lakebase schema + grants | ✅ Always (requires `psql`) |
| App created & deployed | ✅ Always |
| Warehouse detection + SP "Can use" | ✅ Always — best warehouse auto-selected |
| UC catalog/schema visibility for SP | ✅ Always |
| Seed data (demo products/users) | ✅ When `--demo-mode true` (or `--seed true`) |
| End-user UC SELECT grants | ✅ Always — via DataMarket approval flow |
...
Lakebase schema grants weren't applied — usually because `psql` wasn't installed when deploy ran. Install it (`brew install postgresql@16`) and re-run. The script is fully idempotent.
...
SQL Warehouse ID not configured, or the SP doesn't have "Can use". Run deploy again — Step 8 re-applies the grant automatically.
...
Either the Warehouse ID isn't saved in **Manage → Settings**, or the app SP lacks "Can use" on the warehouse. Re-run deploy — it re-applies Step 8 automatically.
...
Pull the latest and re-run. The script is fully idempotent — existing resources are reused, grants are re-applied safely, and the Lakebase hostname is cached so you won't be prompted again.
...
The `--use-bundle true` flag switches the deploy flow to use `databricks bundle deploy` for Lakebase provisioning and app deployment. The script still handles psql schema grants and permissions (DAB can't do those).
...
# Then run grants (required — DAB doesn't do psql or permissions API)
./deploy.sh \
  --profile    my-profile \
  --use-bundle true   # skips redeploy, only runs grant steps
```
